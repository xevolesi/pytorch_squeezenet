...
class SqueezeNetV12(nn.Module):
    def __init__(self, in_channels: int = 3, num_classes: int = 1000, skip_connection_type: str | None = None) -> None:
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(64),
            fire_module_factory(64, 16, 128, True, True, True, complex_skips),
            fire_module_factory(128, 16, 128, True, True, True, simple_skips),
            nn.MaxPool2d((3, 3), stride=2, ceil_mode=True),
            fire_module_factory(128, 32, 256, True, True, True, complex_skips),
            fire_module_factory(256, 32, 256, True, True, True, simple_skips),
            nn.MaxPool2d((3, 3), stride=2, ceil_mode=True),
            fire_module_factory(256, 48, 384, True, True, True, complex_skips),
            fire_module_factory(384, 48, 384, True, True, True, simple_skips),
            fire_module_factory(384, 64, 512, True, True, True, complex_skips),
            fire_module_factory(512, 64, 512, True, True, True, simple_skips),
            nn.Dropout(0.5),
        )
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
